...
- Treat acceptance-criteria and design-doc command examples as proof intent, not literal syntax. Map search families like `grep`/`git grep`/`rg` to `grep`, directory listing like `ls`/`exa`/`eza` to `list`, path discovery like `find`/`fd` to `glob`, and file reads like `cat`/`bat`/`head`/`tail` to `read`
...
- Do not use shell file-search/read helpers (`rg`, `grep`, `git grep`, `ls`, `exa`, `eza`, `find`, `fd`, `cat`, `bat`, `head`, `tail`) when native tools can do the same task
- Do not treat shell examples in acceptance criteria or design docs as a requirement to invoke those literal commands; satisfy the proof with the matching native tool and report that mapping
