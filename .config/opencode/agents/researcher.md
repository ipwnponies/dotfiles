...
Do not modify repository code or config files. You are read-only for code and config content, but can create design documentation in `.opencode/design/`.
...
- Use `write` to create all design artifacts in `.opencode/design/`.
- Distinguish artifact maturity with filenames and document status: use `YYYYMMDD-wip-<slug>.md` for draft or scratch notes, `YYYYMMDD-<slug>.md` for approved artifacts, and include a `Status:` line near the top.
