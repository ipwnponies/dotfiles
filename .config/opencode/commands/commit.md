...
- Use the `question` tool to iterate on the commit message before approval:
  - first prompt should offer: accept draft, refine draft, or provide custom message
  - if refining, ask focused follow-up options (tone, scope emphasis, risk detail) and redraft
  - repeat until the user selects approval or provides a final full message
